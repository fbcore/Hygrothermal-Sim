...
fn get_effective_coef_t(matIdxA: u32, matIdxB: u32, x: u32) -> f32 {\r
    ...
    let isAnyAirA = pA.isAir > 0.5;\r
    let isAnyAirB = pB.isAir > 0.5;\r
    let isCavityA = pA.isAir > 1.5;\r
    let isCavityB = pB.isAir > 1.5;\r
\r
    if (isAnyAirA && isAnyAirB) { \r
        if (isCavityA && isCavityB) {\r
...
\r
    if (isAnyAirA && isAnyAirB) { return AIR_EFFECTIVE_PERM; }\r
    if (isAnyAirA != isAnyAirB) {\r
        let vA = select(permA, AIR_EFFECTIVE_PERM, isAnyAirA);\r
        let vB = select(permB, AIR_EFFECTIVE_PERM, isAnyAirB);\r
...
\r
    ...
    let isCavitySelf = matProps[matIdx].isAir > 1.5;\r
    \r
    var sumT = 0.0;\r
    var sumCoefT = 0.0;\r
    var sumP = 0.0;\r
