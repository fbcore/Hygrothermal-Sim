...
`,d=class{device=null;pipeline=null;width;height;configBuffer=null;materialBuffer=null;matPropsBuffer=null;tempBuffers=null;pressBuffers=null;commonBindGroup=null;bindGroups=null;iteration=0;materialIds;constructor(t,n){this.width=t,this.height=n,this.materialIds=Object.keys(e)}async init(){if(!navigator.gpu)throw Error(`WebGPU not supported`);let e=await navigator.gpu.requestAdapter();if(!e)throw Error(`No adapter found`);this.device=await e.requestDevice();let t=this.device.createShaderModule({code:u});this.pipeline=this.device.createComputePipeline({layout:`auto`,compute:{module:t,entryPoint:`main`}}),this.setupBuffers()}setupBuffers(){if(!this.device)return;let t=this.width*this.height;this.configBuffer=this.device.createBuffer({size:64,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST}),this.materialBuffer=this.device.createBuffer({size:t*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST}),this.matPropsBuffer=this.device.createBuffer({size:this.materialIds.length*16,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST});let n=new Float32Array(this.materialIds.length*4);this.materialIds.forEach((t,r)=>{let i=e[t];n[r*4+0]=i.conductivity,n[r*4+1]=i.permeability,n[r*4+2]=t===`air`?1:0}),this.device.queue.writeBuffer(this.matPropsBuffer,0,n),this.tempBuffers=[this.device.createBuffer({size:t*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST}),this.device.createBuffer({size:t*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST})],this.pressBuffers=[this.device.createBuffer({size:t*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST}),this.device.createBuffer({size:t*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST})],this.commonBindGroup=this.device.createBindGroup({layout:this.pipeline.getBindGroupLayout(0),entries:[{binding:0,resource:{buffer:this.configBuffer}},{binding:1,resource:{buffer:this.materialBuffer}},{binding:2,resource:{buffer:this.matPropsBuffer}}]}),this.bindGroups=[this.device.createBindGroup({layout:this.pipeline.getBindGroupLayout(1),entries:[{binding:0,resource:{buffer:this.tempBuffers[0]}},{binding:1,resource:{buffer:this.tempBuffers[1]}},{binding:2,resource:{buffer:this.pressBuffers[0]}},{binding:3,resource:{buffer:this.pressBuffers[1]}}]}),this.device.createBindGroup({layout:this.pipeline.getBindGroupLayout(1),entries:[{binding:0,resource:{buffer:this.tempBuffers[1]}},{binding:1,resource:{buffer:this.tempBuffers[0]}},{binding:2,resource:{buffer:this.pressBuffers[1]}},{binding:3,resource:{buffer:this.pressBuffers[0]}}]})]}updateData(e,t){if(!this.device||!this.configBuffer||!this.materialBuffer)return;let r=this.width*this.height,i=new Uint32Array(r),a=new Float32Array(r),o=new Float32Array(r);for(let t=0;t<this.height;t++)for(let n=0;n<this.width;n++){let r=t*this.width+n,s=e[t][n];i[r]=this.materialIds.indexOf(s.material),a[r]=s.T,o[r]=s.P}let s=n(t.T_in)*(t.RH_in/100),c=n(t.T_out)*(t.RH_out/100),l=new Float32Array(16),u=new Uint32Array(l.buffer);u[0]=this.width,u[1]=this.height,l[2]=t.T_in,l[3]=t.RH_in,l[4]=t.T_out,l[5]=t.RH_out,l[6]=s,l[7]=c,l[8]=.0025,this.device.queue.writeBuffer(this.configBuffer,0,l),this.device.queue.writeBuffer(this.materialBuffer,0,i),this.device.queue.writeBuffer(this.tempBuffers[0],0,a),this.device.queue.writeBuffer(this.tempBuffers[1],0,a),this.device.queue.writeBuffer(this.pressBuffers[0],0,o),this.device.queue.writeBuffer(this.pressBuffers[1],0,o),this.iteration=0}async run(e){if(!this.device||!this.pipeline||!this.bindGroups)throw Error(`Not initialized`);let t=this.device.createCommandEncoder();for(let n=0;n<e;n++){let e=t.beginComputePass();e.setPipeline(this.pipeline),e.setBindGroup(0,this.commonBindGroup),e.setBindGroup(1,this.bindGroups[this.iteration%2]),e.dispatchWorkgroups(Math.ceil(this.width/16),Math.ceil(this.height/16)),e.end(),this.iteration++}this.device.queue.submit([t.finish()]);let n=this.iteration%2,r=this.iteration%2,i=this.device.createBuffer({size:this.width*this.height*4,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ}),a=this.device.createBuffer({size:this.width*this.height*4,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ}),o=this.device.createCommandEncoder();o.copyBufferToBuffer(this.tempBuffers[n],0,i,0,this.width*this.height*4),o.copyBufferToBuffer(this.pressBuffers[r],0,a,0,this.width*this.height*4),this.device.queue.submit([o.finish()]),await Promise.all([i.mapAsync(GPUMapMode.READ),a.mapAsync(GPUMapMode.READ)]);let s=new Float32Array(i.getMappedRange().slice(0)),c=new Float32Array(a.getMappedRange().slice(0));return i.unmap(),a.unmap(),i.destroy(),a.destroy(),{tempData:s,pressData:c}}destroy(){this.device&&=(this.configBuffer?.destroy(),this.materialBuffer?.destroy(),this.matPropsBuffer?.destroy(),this.tempBuffers&&(this.tempBuffers[0].destroy(),this.tempBuffers[1].destroy()),this.pressBuffers&&(this.pressBuffers[0].destroy(),this.pressBuffers[1].destroy()),this.device.destroy(),null)}};let f=null,p=!1,m=!1,h=!1,g=0,_=0,v=[],y={T_in:20,RH_in:50,T_out:-5,RH_out:80},b=!1;async function x(){if(!(!m||b)){try{if(p&&f){h||=(f.updateData(v,y),!0);let{tempData:e,pressData:t}=await f.run(250);if(m){let n=0,r=0;for(let i=0;i<g*_;i++){let a=Math.floor(i/g),o=i%g,s=Math.abs(e[i]-v[a][o].T),c=Math.abs(t[i]-v[a][o].P);n=Math.max(n,isNaN(s)?0:s),r=Math.max(r,isNaN(c)?0:c),v[a][o].T=e[i],v[a][o].P=t[i]}n<5e-8&&r<5e-4&&(m=!1,self.postMessage({type:`STOPPED`}));let i=e.slice(0).buffer,a=t.slice(0).buffer;b=!0,self.postMessage({type:`SYNC_DATA`,payload:{tempData:new Float32Array(i),pressData:new Float32Array(a),steps:250,residT:n,residP:r}},[i,a])}}else if(m){let e=l(v,g,_,y,50),t=g*_,n=new Float32Array(t),r=new Float32Array(t);for(let e=0;e<_;e++)for(let t=0;t<g;t++){let i=e*g+t;n[i]=v[e][t].T,r[i]=v[e][t].P}e.maxDeltaT<5e-8&&e.maxDeltaP<5e-4&&(m=!1,self.postMessage({type:`STOPPED`}));let i=n.buffer,a=r.buffer;b=!0,self.postMessage({type:`SYNC_DATA`,payload:{tempData:new Float32Array(i),pressData:new Float32Array(a),steps:50,residT:e.maxDeltaT,residP:e.maxDeltaP}},[i,a])}}catch(e){console.error(`Worker simulation error:`,e),m=!1,self.postMessage({type:`ERROR`,payload:e.message})}m&&!b&&setTimeout(x,0)}}self.onmessage=async e=>{let{type:t,payload:n}=e.data;switch(t){case`ACK`:b=!1,m&&x();break;case`INIT`:g=n.width,_=n.height;try{f=new d(g,_),await f.init(),p=!0,self.postMessage({type:`STATUS`,payload:{useGpu:!0,message:`WebGPU Initialized`}})}catch(e){console.warn(`WebGPU init failed in worker:`,e),p=!1,self.postMessage({type:`STATUS`,payload:{useGpu:!1,message:`Fallback to JS Mode`}})}break;case`UPDATE_STATE`:v=n.grid,y=n.boundaries,h=!1;break;case`START`:m||(m=!0,b=!1,x());break;case`STOP`:m=!1;break;case`RESET`:m=!1,h=!1,b=!1;for(let e=0;e<_;e++)for(let t=0;t<g;t++)v[e][t].T=20,v[e][t].P=0;f&&(f.updateData(v,y),h=!0);break;case`DESTROY`:m=!1,f&&=(f.destroy(),null),self.close();break}}})();